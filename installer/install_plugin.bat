...
if %errorlevel% neq 0 (
  ...
  if %errorlevel% equ 0 (
    powershell -NoProfile -Command "Start-Process pwsh -Verb RunAs -ArgumentList '-NoProfile','-ExecutionPolicy','Bypass','-File','%~dp0install_plugin.ps1'"
  ) else (
...
where pwsh.exe >nul 2>&1
if %errorlevel% equ 0 (
  pwsh -NoProfile -ExecutionPolicy Bypass -File "%~dp0install_plugin.ps1"
) else (
